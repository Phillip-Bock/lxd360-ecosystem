...
export function BadgeCard({
    ...
    <>
      ...
      >
        ...
        {isEarned && earned && earned.stackCount > 1 && (
          <div
            className="absolute -top-1 -right-1 px-1.5 py-0.5 rounded-full text-brand-primary text-[10px] font-bold"
          ...
          {!isLocked && (
            <>
              ...
              {!isEarned && badge.xpReward > 0 && (
                <p className="mt-2 text-[10px]" style={{ color: 'var(--lxd-blue-light)' }}>
                  +{badge.xpReward} XP
                </p>
              )}
            </>
  ...
  if (onClick) {
    return (
      <button
        ...
        onMouseEnter={(e) => {
          e.currentTarget.style.borderColor = 'rgba(71, 157, 255, 0.5)';
          e.currentTarget.style.backgroundColor = '#0f1d2e';
        }}
        onMouseLeave={(e) => {
  ...
  return (
    <div
      className={cn('flex flex-col items-center rounded-2xl', 'border', sizes.container, className)}
      style={{
        backgroundColor: '#0d1829',
        borderColor: 'rgba(30, 58, 95, 0.5)',
      }}
