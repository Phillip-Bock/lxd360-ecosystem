...
function Code({ className, code, ...props }: CodeProps): React.JSX.Element {
  return (
...
function CodeHeader({
  ...
  copyButton = false,
  ...props
}: CodeHeaderProps): React.JSX.Element {
  const { code } = useCode();
...
function CodeBlock({ cursor, className, ...props }: CodeBlockProps): React.JSX.Element {
