...
function Highlight<T extends React.ElementType = 'div'>({
  ref,
  ...props
}: HighlightProps<T>): React.ReactElement {
  const {
    as = 'div',
    children,
    value,
  ...
  const Component = as as React.ElementType<React.ComponentPropsWithRef<'div'>>;
  ...
  const safeSetActiveValue = (id: string | null): void => {
    setActiveValue((prev) => {
      if (prev !== id) {
        onValueChange?.(id);
        return id;
  ...
  React.useEffect(() => {
    safeSetBoundsRef.current = (bounds: DOMRect): void => {
...
function HighlightItem<T extends React.ElementType>({
  ...
  forceUpdateBounds,
  ...props
}: HighlightItemProps<T>): React.ReactNode {
