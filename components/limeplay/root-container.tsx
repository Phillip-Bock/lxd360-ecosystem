...
export type RootContainerPropsDocs = Pick<RootContainerProps, 'height' | 'width'>;

export const RootContainer = React.forwardRef<HTMLDivElement, RootContainerProps>(
  (props, forwardedRef) => {
    ...
    const setPlayerContainerRef = useMediaStore((state) => state.setPlayerContainerRef);
    const aspectRatio = React.useMemo(() => calculateAspectRatio(width, height), [height, width])
      ?.split(':')
      .join('/');
