...
export type CaptionsControlPropsDocs = Pick<CaptionsControlProps, 'asChild' | 'shortcut'>;

export const CaptionsControl = React.forwardRef<HTMLButtonElement, CaptionsControlProps>(
  (props, forwardedRef) => {
...
export type CaptionsContainerPropsDocs = Pick<CaptionsContainerProps, 'fontScale'>;
...
export const CaptionsContainer = React.forwardRef<HTMLDivElement, CaptionsContainerProps>(
  (props, ref) => {
    const { className, fontScale, ...etc } = props;
    const player = useMediaStore((state) => state.player);
    const setContainerElement = useMediaStore((state) => state.setTextTrackContainerElement);
    ...
    return (
      <div
        className={cn('relative flex w-full grow flex-col justify-end text-lg', className)}
