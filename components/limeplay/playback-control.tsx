...
export type PlaybackControlPropsDocs = Pick<PlaybackControlProps, 'asChild' | 'shortcut'>;
...
export const PlaybackControl = React.forwardRef<HTMLButtonElement, PlaybackControlProps>(
  (props, forwardedRef) => {
    ...
    const isDisabled =
      readyState < MediaReadyState.HAVE_CURRENT_DATA || status === 'buffering' || userDisabled;
    ...
    const getDefaultAriaLabel = () => {
      ...
      const label =
        status === 'ended' ? labels.ended : status === 'playing' ? labels.playing : labels.default;
