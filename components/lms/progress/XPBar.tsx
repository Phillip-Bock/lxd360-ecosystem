...
export function XPBar({
  ...
  return (
    <div
      className={cn(
        'rounded-2xl bg-[var(--studio-bg)] border border-[var(--lxd-dark-surface-alt)]/50',
        sizes.container,
        className,
      )}
    >
      <div className="flex items-center gap-3">
        ...
        <div
          className={cn(
            ...
            'bg-linear-to-br from-[var(--studio-accent)] to-[var(--lxd-purple-light)] text-brand-primary',
            sizes.level,
          )}
        >
          {level}
          ...
          {showDetails && xpToNextLevel > 0 && (
            <div className={cn('flex items-center justify-between mt-1', sizes.text)}>
              <span className="text-muted-foreground">
                <Sparkles className="w-3 h-3 inline mr-1 text-[var(--studio-accent)]" />
