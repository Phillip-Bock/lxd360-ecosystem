 ...
 * Content can be managed via CMS with hardcoded fallbacks.
 *
 * 5-Column Layout: Brand | Policies | Compliance | Legal | Quick Links
 */
...
const DEFAULT_QUICK_LINKS: LinkType[] = [
  ...
  { _key: '6', label: 'Log In', href: '/auth/login' },
];
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
          ...
          <div className="bg-lxd-light-card/80 dark:bg-lxd-dark-surface/60 backdrop-blur-lg rounded-xl p-5 border-2 border-lxd-light-border/50 dark:border-lxd-purple-dark/30">
            ...
            <ul className="space-y-1.5">
              {legal.map((item) => {
                // Use root-level routes for terms and privacy, policies route for others
                const href =
                  item.slug === 'terms' || item.slug === 'privacy'
                    ? `/${item.slug}`
                    : `/policies/${item.slug}`;
                return (
                  <li key={item._key}>
