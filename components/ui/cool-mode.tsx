...
const applyParticleEffect = (element: HTMLElement, options?: CoolParticleOptions): (() => void) => {
  ...
  function generateParticle() {
    ...
    if (particleType === 'circle') {
      ...
      // Handle URL-based images using DOM API
      const imgElement = document.createElement('img');
      imgElement.src = particleType;
      imgElement.width = size;
      imgElement.height = size;
      imgElement.style.borderRadius = '50%';
      imgElement.alt = '';
      particle.appendChild(imgElement);
    } else {
      // Handle emoji or text characters using DOM API
      const fontSizeMultiplier = 3; // Make emojis 3x bigger
      const emojiSize = size * fontSizeMultiplier;
      const emojiContainer = document.createElement('div');
      emojiContainer.style.fontSize = `${emojiSize}px`;
      emojiContainer.style.lineHeight = '1';
      emojiContainer.style.textAlign = 'center';
      emojiContainer.style.width = `${size}px`;
      emojiContainer.style.height = `${size}px`;
      emojiContainer.style.display = 'flex';
      emojiContainer.style.alignItems = 'center';
      emojiContainer.style.justifyContent = 'center';
      emojiContainer.style.transform = `scale(${fontSizeMultiplier})`;
      emojiContainer.style.transformOrigin = 'center';
      emojiContainer.textContent = particleType;
      particle.appendChild(emojiContainer);
    }
