  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <div className={cn(
          'p-2 rounded-full',
          fluencyZone === 'struggle' ? 'animate-pulse' : ''
        )}>
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            ...
            <span className={cn('text-lg font-mono', style.text)}>
              {formatTime(displayTime)}
            </span>
          </div>
          ...
          <div className="mt-3 h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
            <motion.div
              className={cn(
                'h-full rounded-full',
                fluencyZone === 'too_fast' ? 'bg-blue-500' :
                fluencyZone === 'fluency' ? 'bg-green-500' :
                fluencyZone === 'thinking' ? 'bg-yellow-500' :
                'bg-red-500'
              )}
              initial={{ width: 0 }}
              animate={{ 
