...
export const DeviceTypeSchema = z.enum(['desktop', 'tablet', 'mobile', 'vr', 'unknown']);
export type DeviceType = z.infer<typeof DeviceTypeSchema>;

export const NetworkQualitySchema = z.enum(['offline', 'slow-2g', '2g', '3g', '4g', '5g']);
...
export const InspireExtensionsSchema = z
  .object({
...
export function createInspireContextExtensions(data: {
  ...
  if (data.consentTier !== undefined)
    extensions[INSPIRE_EXTENSIONS.CONSENT_TIER] = data.consentTier;
...
export function createInspireBehavioralExtensions(data: {
  ...
  if (data.timeToFirstAction !== undefined)
    extensions[INSPIRE_EXTENSIONS.TIME_TO_FIRST_ACTION] = data.timeToFirstAction;
  if (data.revisionCount !== undefined)
    extensions[INSPIRE_EXTENSIONS.REVISION_COUNT] = data.revisionCount;
  if (data.distractorsTouched)
    extensions[INSPIRE_EXTENSIONS.DISTRACTORS_TOUCHED] = data.distractorsTouched;
  if (data.rageClicks !== undefined) extensions[INSPIRE_EXTENSIONS.RAGE_CLICKS] = data.rageClicks;
  if (data.confidenceRating !== undefined)
    extensions[INSPIRE_EXTENSIONS.CONFIDENCE_RATING] = data.confidenceRating;
...
export function createInspireAIExtensions(data: {
  ...
  if (data.recommended !== undefined)
    extensions[INSPIRE_EXTENSIONS.AI_RECOMMENDED] = data.recommended;
  ...
  if (data.learnerOverride !== undefined)
    extensions[INSPIRE_EXTENSIONS.LEARNER_OVERRIDE] = data.learnerOverride;
...
export function createInspireSkillExtensions(data: {
  ...
  if (data.masteryEstimate !== undefined)
    extensions[INSPIRE_EXTENSIONS.MASTERY_ESTIMATE] = data.masteryEstimate;
  if (data.masteryLevel) extensions[INSPIRE_EXTENSIONS.MASTERY_LEVEL] = data.masteryLevel;
  if (data.prerequisiteSkills)
    extensions[INSPIRE_EXTENSIONS.PREREQUISITE_SKILLS] = data.prerequisiteSkills;
