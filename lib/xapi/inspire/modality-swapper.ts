...
export function ModalitySwapperProvider({
  ...
  // Send xAPI statement
  const sendXAPIStatement = useCallback(async (statement: unknown) => {
    try {
  ...
  }, [learnerProfile, actor, sessionId, tenantId, consentTier, profileRef, sendXAPIStatement, onSwap]);
  
  // Override AI recommendation
  const overrideRecommendation = useCallback(async (preferredModality: Modality, reason?: string) => {
    const recommendation = learnerProfile?.activeRecommendation;
    if (!recommendation) return;
  ...
  const getModalityForBlock = useCallback((blockId: string, defaultModality: Modality): Modality => {
  ...
  const shouldShowSwapPrompt = useCallback((blockId: string): boolean => {
    const recommendation = learnerProfile?.activeRecommendation;
  ...
  return (
    <ModalitySwapperContext.Provider value={value}>
      {children}
    </ModalitySwapperContext.Provider>
  );
}
...
export function ModalitySwapPrompt({
  ...
  if (compact) {
    return (
      <div className={`inline-flex items-center gap-2 rounded-full bg-cyan-500/10 px-3 py-1 text-sm ${className}`}>
        <span className="text-cyan-400">
          💡 Try {activeRecommendation.toModality}?
        </span>
        <button
          type="button"
  ...
  return (
    <div className={`rounded-lg border border-cyan-500/30 bg-gradient-to-br from-cyan-500/10 to-purple-500/10 p-4 ${className}`}>
