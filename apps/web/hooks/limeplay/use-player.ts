...
export type MediaReadyState = (typeof MediaReadyState)[keyof typeof MediaReadyState];
...
export const createPlayerStore: StateCreator<PlayerStore, [], [], PlayerStore> = (set) => ({
  canPlay: false,
  ...
  setMediaRef: (mediaRef: React.RefObject<HTMLMediaElement>) => set({ mediaRef }),
...
export function usePlayerStates() {
  ...
  React.useEffect(() => {
    ...
    const setInitialState = () => {
      ...
      const status: MediaStatus = isBuffering ? 'buffering' : media.paused ? 'paused' : 'playing';
