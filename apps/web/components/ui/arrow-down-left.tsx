...
const ArrowDownLeftIcon = forwardRef<ArrowDownLeftIconHandle, ArrowDownLeftIconProps>(
  ({ onMouseEnter, onMouseLeave, className, size = 28, ...props }, ref) => {
    const controls = useAnimation();
    const isControlledRef = useRef(false);

    useImperativeHandle(ref, (): ArrowDownLeftIconHandle => {
    ...
    return (
      ...
      >
        <svg
          ...
          aria-hidden="true"
        >
          <motion.path d="M17 17H7V7" variants={HEAD_VARIANTS} animate={controls} />
          <motion.path d="M7 17 L12 12" variants={SHAFT_VARIANTS} animate={controls} />
          <path d="M17 7 L12 12" />
        </svg>
      </div>
    );
