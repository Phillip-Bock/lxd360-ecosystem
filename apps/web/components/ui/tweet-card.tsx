...
export const TweetMedia = ({ tweet }: { tweet: EnrichedTweet }) => {
  ...
  return (
    <div className="flex flex-1 items-center justify-center">
      ...
      {(() => {
        if (tweet.video || tweet.photos) return null;
        const tweetWithCard = tweet as EnrichedTweet & {
          card?: {
            binding_values?: { thumbnail_image_large?: { image_value?: { url?: string } } };
          };
        };
        const card = tweetWithCard.card;
        const thumbnailUrl = card?.binding_values?.thumbnail_image_large?.image_value?.url;
