...
const CloudSnowIcon = forwardRef<CloudSnowIconHandle, CloudSnowIconProps>(
  ({ onMouseEnter, onMouseLeave, className, size = 28, ...props }, ref) => {
    ...
    useImperativeHandle(ref, (): CloudSnowIconHandle => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <motion.g variants={SNOWFLAKE_VARIANTS} animate={controls} initial="normal">
            {SNOWFLAKE_PATH.map((path) => (
              <motion.path variants={SNOWFLAKE_CHILD_VARIANTS} key={path.id} d={path.d} />
            ))}
          </motion.g>
        </svg>
      </div>
