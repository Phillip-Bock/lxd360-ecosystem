...
import { Apple, Github, Sparkles } from 'lucide-react';
  ...
  return (
    <div
      className={cn(
        ...
        'dark:border-neutral-800/50 dark:bg-muted-foreground',
        'border-neutral-200',
      )}
    >
      <FrameworkCard />
      <div className="px-4 pb-4">
        <div className="text-sm font-semibold text-neutral-900 dark:text-white">{cardTitle}</div>
        <div className="mt-2 text-xs text-neutral-600 dark:text-neutral-400">{cardDescription}</div>
      </div>
    </div>
  );
};
...
const FrameworkCard = () => {
  ...
  useEffect(() => {
    const cycleAnimations = async () => {
      ...
      while (true) {
        ...
        await new Promise((resolve) => setTimeout(resolve, transitionDuration + delayBetweenCards));
  ...
  const cardClasses =
    'flex aspect-square items-center justify-center rounded-md border bg-linear-to-b from-neutral-50 to-neutral-100 p-4 ' +
    'dark:border-neutral-800 dark:from-muted-foreground dark:to-muted-foreground ' +
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 h-3 w-full bg-linear-to-t from-white to-transparent dark:from-muted-foreground" />
    </div>
  );
};
