...
function CarouselProvider({
  ...
  onIndexChange,
  disableDrag = false,
}: CarouselProviderProps): React.JSX.Element {
  const [index, setIndex] = useState<number>(initialIndex);
  const [itemsCount, setItemsCount] = useState<number>(0);

  const handleSetIndex = (newIndex: number): void => {
    setIndex(newIndex);
...
function CarouselNavigation({
  ...
  classNameButton,
  alwaysShow,
}: CarouselNavigationProps): React.JSX.Element {
...
function CarouselIndicator({
  className,
  classNameButton,
}: CarouselIndicatorProps): React.JSX.Element {
...
function CarouselItem({ children, className }: CarouselItemProps): React.JSX.Element {
  return (
