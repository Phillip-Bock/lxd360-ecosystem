        ...
        return (
          <button
            ...
            className={cn(
              'w-full flex items-center gap-3 p-4 rounded-xl border transition-all text-left',
              isSelected && !showFeedback
                ? 'bg-[var(--studio-accent)]/10 border-[var(--studio-accent)]/50'
                : 'bg-[var(--studio-bg)] border-[var(--lxd-dark-surface-alt)]/50 hover:border-[var(--studio-accent)]/30',
              isCorrect && 'bg-brand-success/10 border-emerald-500/50',
              isWrong && 'bg-brand-error/10 border-brand-error/50',
              disabled && 'cursor-default',
            )}
        ...
        return (
          <button
            ...
            className={cn(
              ...
              isSelected && !showFeedback
                ? 'bg-[var(--studio-accent)]/10 border-[var(--studio-accent)]/50 text-[var(--studio-accent)]'
                : 'bg-[var(--studio-bg)] border-[var(--lxd-dark-surface-alt)]/50 text-brand-primary hover:border-[var(--studio-accent)]/30',
  ...
  return (
    <div>
      <input
        ...
        className={cn(
          'w-full p-4 bg-[var(--studio-bg)] border rounded-xl text-brand-primary placeholder-muted-foreground focus:outline-hidden transition-colors',
          showFeedback
            ? question.correctAnswers?.some((a) => a.toLowerCase() === answer.toLowerCase())
              ? 'border-emerald-500/50'
              : 'border-brand-error/50'
            : 'border-[var(--lxd-dark-surface-alt)]/50 focus:border-[var(--studio-accent)]/50',
        )}
      />
      {question.maxLength && (
  ...
  return (
    <div>
      <textarea
        ...
        rows={8}
        className="w-full p-4 bg-[var(--studio-bg)] border border-[var(--lxd-dark-surface-alt)]/50 rounded-xl text-brand-primary placeholder-muted-foreground focus:outline-hidden focus:border-[var(--studio-accent)]/50 resize-none"
      />
  ...
  return (
    <div className="flex items-center gap-3">
      <input
        ...
        className={cn(
          'flex-1 p-4 bg-[var(--studio-bg)] border rounded-xl text-brand-primary placeholder-muted-foreground focus:outline-hidden transition-colors',
          showFeedback
            ? isCorrect
              ? 'border-emerald-500/50'
              : 'border-brand-error/50'
...
function Likert({
  ...
  const handleSelect = (statementId: string, value: number): void => {
    if (disabled) return;
              ...
              <button
                ...
                className={cn(
                  ...
                  answer[statement.id] === s.value
                    ? 'bg-[var(--studio-accent)]/20 border-[var(--studio-accent)]/50'
                    : 'bg-[var(--studio-bg)] border-[var(--lxd-dark-surface-alt)]/50 hover:border-[var(--studio-accent)]/30',
                )}
              >
                {answer[statement.id] === s.value && (
                  <div className="w-3 h-3 bg-[var(--studio-accent)] rounded-full mx-auto" />
                )}
              </button>
            ))}
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-3">
        <span className="shrink-0 w-8 h-8 rounded-lg bg-[var(--studio-accent)]/20 flex items-center justify-center text-[var(--studio-accent)] font-bold">
          {questionNumber}
        </span>
        <div className="flex-1">
      ...
      {question.hint && !showFeedback && (
        <button
          ...
          className="flex items-center gap-2 text-sm text-[var(--studio-accent)] hover:underline"
        >
      ...
      {showHint && question.hint && (
        <div className="p-3 bg-[var(--studio-accent)]/10 border border-[var(--studio-accent)]/30 rounded-lg text-sm text-[var(--studio-accent)]">
          {question.hint}
        </div>
      )}
