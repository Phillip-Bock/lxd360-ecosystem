...
import { ProgressBar } from '@/components/lms/progress/progress-bar';
import { cn } from '@/lib/utils';
import { type Question, QuestionRenderer } from './question-renderer';
...
export function QuizPlayer({
  ...
  //   randomizeOptions: _randomizeOptions = false,
  onComplete,
  onExit,
  ...
  // Ref to store handleSubmit function to avoid circular dependency
  ...
  // Handle answer
  const handleAnswer = (answer: unknown): void => {
    setAnswers((prev) => ({
      ...prev,
      ...
      switch (question.type) {
        ...
        case 'multiple_choice_multiple':
          if ('options' in question && Array.isArray(question.options)) {
            ...
            const answerArray: string[] = Array.isArray(answer) ? (answer as string[]) : [];
            isCorrect =
        ...
        case 'short_answer':
          if ('correctAnswers' in question && Array.isArray(question.correctAnswers)) {
            ...
            isCorrect = question.correctAnswers.some(
              (a: string) => a.toLowerCase() === answerStr.toLowerCase(),
            );
          }
          break;
        case 'numeric':
          if ('correctAnswer' in question && typeof question.correctAnswer === 'number') {
            ...
            const tolerance =
              'tolerance' in question && typeof question.tolerance === 'number'
                ? question.tolerance
                : 0;
            isCorrect =
              !Number.isNaN(numAnswer) && Math.abs(numAnswer - correctAnswer) <= tolerance;
          }
          break;
