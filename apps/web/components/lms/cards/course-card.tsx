...
export function CourseCard({
  ...
  if (variant === 'compact') {
    ...
    return (
      <Link href={`/lms/catalog/${course.id}`} className={compactClassName}>
        {compactContent}
      </Link>
    );
  }
      ...
      <div className="flex-1 flex flex-col justify-between py-1">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs text-[var(--studio-accent)]">{course.category.name}</span>
            {isRequired && (
              <span className="px-2 py-0.5 text-[10px] font-medium rounded-full bg-brand-error/10 text-brand-error border border-brand-error/20">
                Required
              </span>
          ...
          <h3 className="text-lg font-semibold text-brand-primary group-hover:text-[var(--studio-accent)] transition-colors line-clamp-2">
            {course.title}
          </h3>
  ...
  if (variant === 'horizontal') {
    ...
    return (
      <Link href={`/lms/catalog/${course.id}`} className={horizontalClassName}>
        {horizontalContent}
      </Link>
    );
  }
  ...
  const defaultContent = (
    <>
      ...
      >
        ...
        {!isEnrolled && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity">
            <div className="p-3 rounded-full bg-[var(--studio-accent)] text-brand-primary">
      ...
      <div className={cn('flex flex-col flex-1 p-4', variant === 'featured' && 'md:p-6')}>
        ...
        <h3
          className={cn(
            'font-semibold text-brand-primary group-hover:text-[var(--studio-accent)] transition-colors',
        ...
        {!isEnrolled && !course.isFree && course.price && (
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[var(--lxd-dark-surface-alt)]/50">
            {course.salePrice ? (
              <>
  ...
  return (
    <Link href={`/lms/catalog/${course.id}`} className={defaultClassName}>
      {defaultContent}
    </Link>
  );
}
