...
function CursorProvider({ children, global = false }: CursorProviderProps): React.JSX.Element {
  ...
  React.useEffect(() => {
    ...
    if (global) {
      ...
      const handlePointerOut = (e: PointerEvent | MouseEvent): void => {
...
function CursorContainer({
  ...
  asChild = false,
  ...props
}: CursorContainerProps): React.JSX.Element {
...
function Cursor({ ref, asChild = false, style, ...props }: CursorProps): React.JSX.Element {
  ...
  const positionStyle = global ? 'fixed' : 'absolute';
  ...
  const commonProps = {
    ...
    style: {
      ...
      zIndex: 9999,
      position: positionStyle as 'fixed' | 'absolute',
      top: y,
      left: x,
      ...style,
    },
  ...
  return (
    <AnimatePresence>
      {active && (asChild ? <Slot {...commonProps} /> : <motion.div {...commonProps} />)}
    </AnimatePresence>
  );
}
...
function CursorFollow({
  ...
  ...props
}: CursorFollowProps): React.JSX.Element {
  ...
  const followPositionStyle = global ? 'fixed' : 'absolute';
  ...
  const commonProps = {
    ...
    style: {
      ...
      pointerEvents: 'none' as const,
      zIndex: 9998,
      position: followPositionStyle as 'fixed' | 'absolute',
      top: springY,
      left: springX,
      ...style,
    },
