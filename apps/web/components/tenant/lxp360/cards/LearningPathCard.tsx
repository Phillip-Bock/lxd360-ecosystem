    ...
    return isButton ? (
      <button
        ...
        className={cn(
          ...
          'bg-studio-bg border border-studio-surface/50',
          'hover:border-(--lxd-blue-light)/50 hover:bg-studio-bg',
          'transition-all duration-200 text-left w-full',
          className,
        )}
      >
        <div className="relative w-16 h-16 rounded-xl overflow-hidden shrink-0 bg-linear-to-br from-(--lxd-blue-light)/20 to-(--brand-secondary)/20 flex items-center justify-center">
          {path.thumbnail ? (
            <Image src={path.thumbnail} alt={path.title} fill className="object-cover" />
          ) : (
            <Award className="w-8 h-8 text-(--lxd-blue-light)" />
          )}
          {isCompleted && (
        ...
        <ChevronRight className="w-5 h-5 text-brand-muted group-hover:text-(--lxd-blue-light)" />
      </button>
    ) : (
      <Link
        href={`/lms/learning-paths/${path.id}`}
        className={cn(
          'group flex items-center gap-4 p-3 rounded-xl',
          'bg-studio-bg border border-studio-surface/50',
          'hover:border-(--lxd-blue-light)/50 hover:bg-studio-bg',
      ...
      >
        ...
        <div className="relative w-48 h-32 rounded-xl overflow-hidden shrink-0">
          {path.coverImage || path.thumbnail ? (
            ...
            <div className="absolute inset-0 bg-linear-to-br from-(--lxd-blue-light)/30 to-(--brand-secondary)/30 flex items-center justify-center">
              <Award className="w-12 h-12 text-(--lxd-blue-light)" />
            </div>
          )}
          {isCompleted && (
            <div className="absolute inset-0 bg-brand-success/20 flex items-center justify-center">
        ...
        <div className="flex-1 flex flex-col justify-between py-1">
          <div>
            ...
            <h3 className="text-lg font-semibold text-brand-primary group-hover:text-(--lxd-blue-light) transition-colors">
              {path.title}
            </h3>
    ...
    >
      ...
      <div className="relative aspect-[16/9] overflow-hidden">
        {path.coverImage || path.thumbnail ? (
          ...
          <div className="absolute inset-0 bg-linear-to-br from-(--lxd-blue-light)/30 to-(--brand-secondary)/30 flex items-center justify-center">
            <Award className="w-16 h-16 text-(--lxd-blue-light)" />
          </div>
        )}
      ...
      <div className="flex flex-col flex-1 p-4">
        <h3 className="text-base font-semibold text-brand-primary group-hover:text-(--lxd-blue-light) transition-colors line-clamp-2">
          {path.title}
        </h3>
        ...
        {path.skills.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-3">
            {path.skills.slice(0, 3).map((skill) => (
              <span
                key={skill.skillId}
                className="px-2 py-0.5 text-[10px] rounded-full bg-(--lxd-blue-light)/10 text-(--lxd-blue-light) border border-(--lxd-blue-light)/20"
              >
                {skill.skillName}
              </span>
            ))}
