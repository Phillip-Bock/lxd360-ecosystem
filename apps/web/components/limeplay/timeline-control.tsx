...
import { MediaReadyState } from '@/hooks/limeplay/use-player';
    ...
    onPointerDown: (_progress, event) => {
      if (player) {
        ...
        const liveSeekTime = isLive
          ? seekRange.start + (newTime / duration) * (seekRange.end - seekRange.start)
          : newTime;
...
>((props, ref) => {
  ...
  return (
    <SliderPrimitive.Range
      className={cn('absolute h-full w-(--lp-played-width)! rounded-s-full bg-primary', className)}
      ref={ref}
      style={
        {
...
export type TimelineThumbPropsDocs = Pick<ThumbProps, 'position' | 'showWithHover'>;

interface BufferedProps extends React.ComponentProps<typeof SliderPrimitive.Track> {
  /**
...
interface ThumbProps extends React.ComponentProps<typeof SliderPrimitive.Thumb> {
  /**
...
export const Buffered = React.forwardRef<HTMLDivElement, BufferedProps>((props, ref) => {
  ...
  return (
    <div className={cn('absolute size-full', className)} ref={ref} {...etc}>
      {normalizedPercentages.map(({ startPercent, widthPercent }, index) => (
        <SliderPrimitive.Range
          className={cn(
            `absolute left-(--lp-buffered-start)! h-full w-(--lp-buffered-width)! bg-foreground/30`,
