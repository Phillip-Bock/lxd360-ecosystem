 ...
 */
export const Elapsed = React.forwardRef<HTMLTimeElement, React.HTMLProps<HTMLTimeElement>>(
  (props, forwardedRef) => {
 ...
 * @note formatted as HH:MM:SS where HH only shows if > 1 hour
 */
export const Remaining = React.forwardRef<HTMLTimeElement, React.HTMLProps<HTMLTimeElement>>(
  (props, forwardedRef) => {
    const { className, ...etc } = props;
    const duration = useMediaStore((s) => s.duration);
    const currentTime = useMediaStore((s) => s.currentTime);
 ...
 */
export const Duration = React.forwardRef<HTMLTimeElement, React.HTMLProps<HTMLTimeElement>>(
  (props, forwardedRef) => {
    const { className, ...etc } = props;
    const duration = useMediaStore((s) => s.duration);
    const player = useMediaStore((s) => s.player);
...
export const HoverTime = React.forwardRef<HTMLTimeElement, React.HTMLProps<HTMLTimeElement>>(
  (props, forwardedRef) => {
    const { className, ...etc } = props;
...
export const LiveLatency = React.forwardRef<HTMLTimeElement, React.HTMLProps<HTMLTimeElement>>(
  (props, forwardedRef) => {
    const { className, ...etc } = props;
